var $User = (function () {
  ...
  oP = {
    ...
    AddZombiesFlag: function (d) {
      ...
      for (e in c) {
        Math.floor(e) < a
          ? SetStyle(c[e], { display: "block", left: 150 - (e - 1) * b + "px" })
          : SetBlock(c[e]);
      }
      oGd.MoveBullet();
      g.ReadyFlag = 1;
                ...
                ? ((a = c.Speed),
                  !c.WalkDirection
                    ? (c.AttackedRX -= a) < 100
                      ...
                      : (($(g).style.left = Math.floor((c.X -= a)) + "px"),
                        b(c))
                    : (c.AttackedLX += a) > oS.W
                    ? (i.splice(m, 1), c.Die(2), d(c))
                    : ((c.ZX = c.AttackedRX += a),
                      ($(g).style.left = Math.ceil((c.X += a)) + "px"),
                      b(c)))
                : b(c))
            : b(c);
        }
    ...
    switch (oS.Chose) {
      ...
      case -1:
        var j = p[1],
          ...
          q = oS.MPID;
        q != l &&
          (q && SetAlpha($(q).childNodes[1], 100, 1),
  ...
  SelectCard = function (f) {
    ...
    if (!i.Select) {
      ...
      i.Select = 0;
      !--ArPCard.SelNum &&
        ((g.disabled = "disabled"), (g.style.color = "#888"));
    ...
    while (b--) {
      ...
      switch (a) {
        case 7500:
          d.CDReady = 1;
          e <= oS.SunNum &&
            ((d.SunReady = 1), ($(d.DID).firstChild.src = c.PicArr[c.CardGif]));
          break;
        case 30000:
      ...
      while (a--) {
        (b = (c = ArCard[a]).PName.prototype).SunNum > oS.SunNum
          ? (c.SunReady && (c.SunReady = 0),
            ($(c.DID).firstChild.src = b.PicArr[b.CardGif + 1]))
          : (!c.SunReady && (c.SunReady = 1),
            c.CDReady && ($(c.DID).firstChild.src = b.PicArr[b.CardGif]));
      }
    } else {
      while (a--) {
        (b = (c = ArCard[a]).PName.prototype).SunNum > oS.SunNum
  ...
  ChosePlant = function (a, f) {
    ...
    while (c--) {
      $((b = ArCard[c]).DID).firstChild.src = (d = b.PName.prototype).PicArr[
        d.CardGif + 1
      ];
    }
    SetNone($("dTitle"));
      ...
      oSym.addTask(
        ...
        function (c, e, d) {
          ...
          c > 252
            ...
            : oSym.addTask(d, ClearChild, [$("FinalWave")]);
        },
        [756, 213, b]
      );
    };
  ...
  LawnMowerKill = function (b) {
    ...
    (function () {
      ...
      (d += 10) > a
        ? ClearChild(c)
        : ((c.style.left = d + "px"), oSym.addTask(1, arguments.callee, []));
    })();
  },
  GameOver = function () {
    NewImg(
  ...
  (AppearTombstones = function (j, d, h) {
    ...
    while (h--) {
      ...
      NewEle(
        "",
        "div",
        "background-position:-" + 86 * b + "px -" + 91 * f + "px",
        { className: "Tom1" },
        a
      );
      NewEle(
